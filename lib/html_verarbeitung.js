    ...
    return new Promise(async (resolve) => {
       ...
       //if(task.hasOwnProperty('content')){task.content}else{""};
        try {
            ...
            //neue felder ab 0.9:
            if (adapter.config.html_due_date) { if(task.hasOwnProperty('due')){ HTMLstring = HTMLstring + task.due.date + '</td><td>'}else{ HTMLstring = HTMLstring + " " + '</td><td>'} };
            if (adapter.config.html_due_string) { { if(task.hasOwnProperty('due')){ HTMLstring = HTMLstring + task.due.string + '</td><td>'}else{HTMLstring = HTMLstring + " " + '</td><td>'} }; };
            if (adapter.config.html_due_recurring) { { if(task.hasOwnProperty('due')){ HTMLstring = HTMLstring + task.due.recurring + '</td><td>'}else{HTMLstring = HTMLstring + " " + '</td><td>'} }; };
